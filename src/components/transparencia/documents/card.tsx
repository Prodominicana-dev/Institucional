...
export default function DocsCard({doc}: {doc: {id: string, name: string, date: Date, size: string, url: string, sectionId: string}}) {
  const handleClick = async () => {
    try {
      ...
      console.log(`${process.env.NEXT_PUBLIC_API_URL}/files/docs/${doc.sectionId}/${doc.name}`)
    ...
    const icon = extension === 'pdf' ? 'pdf' :  extension === 'xlsx' ? 'xls' : doc.url !== '' && 'url'
                    ...
                    >
                      <div className="flex flex-col sm:flex-row items-center gap-3">
                        ...
                        <div className="space-y-3 sm:space-y-0">
                          <p
                            className="text-lg text-black uppercase font-bold font-montserrat line-clamp-2"
                          >
                        {doc.name}
                          </p>
                          <div className="w-full flex gap-5">
                            <p
                              className={`${!doc.url ? "block" : "hidden"} text-black font-medium w-auto font-montserrat`}
                            >
                              {size} MB
                            </p>
                            <p
