...
export default function SidebarMenu({title, subsections, id} : {title: string, subsections: any, id: string}) {
    ...
    <Menu allowHover={true} placement='right-start'>
        <MenuHandler>
            <button className='w-11/12 h-14 bg-transparent hover:bg-white/30 duration-300 flex flex-row items-center justify-between rounded-lg px-5 group'>
                ...
                <ChevronDownIcon className={`${open ? "rotate-90" : "rotate-0 "} group-hover:-rotate-90 duration-300 transform size-6 text-white`} />
            </button>
        </MenuHandler>
        <MenuList placeholder={undefined} className='min-w-[24rem]'>
            {subsections.map((subsection: any, index: number) => (
                <MenuItem placeholder={undefined} key={index} className='w-full'>
                    {subsection.type === 'url' && ( <MenuLink title={subsection.name} link={subsection.url} /> ) }
                    {subsection.type === 'document' && (<MenuDocs title={subsection.name} sectionId={id} subsectionId={subsection.id} />) }
                </MenuItem>
            ))}
        </MenuList>
        </Menu>
