...
export default function MenuDocs({title, sectionId, subsectionId}: {title: string, sectionId: string, subsectionId: string}) {
  return (
    <Link href={`/transparency/${sectionId}/${subsectionId}`} target='_blank' className='w-11/12 h-10 bg-transparent hover:bg-white/30 duration-300 flex flex-row items-center justify-between rounded-lg px-0'>
