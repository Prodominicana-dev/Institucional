...
export function EditNewsDialog({
  ...
  /* A diferencia del modal de creación, acá siempre hay contenido cargado
     (viene de la noticia existente), así que confirmar antes de cerrar
     evita perder ediciones a mitad de camino. */
  const handleOpenChange = (nextOpen: boolean) => {
    if (!nextOpen && !submitLoading) {
      const confirmed = window.confirm(
        "¿Deseas cerrar sin guardar los cambios?"
      );
      if (!confirmed) return;
    }
    handler();
  ...
  /* "Eliminar" descarta solo la selección nueva y vuelve a mostrar la
     portada que ya tenía la noticia, no la borra del todo: esta pantalla
     no tiene forma de dejar una noticia sin portada. */
  ...
  const handleButton = async () => {
    ...
    if (
      ...
      /* Se comprime la portada salvo que el redactor pida el original.
         Solo aplica si eligió una imagen nueva; si no, se conserva la actual. */
  ...
  const existingCoverSrc =
    files.length === 0 && cover
      ? `${process.env.NEXT_PUBLIC_API_URL}/news/images/${id}/${cover}`
      : undefined;

  const newCoverBlobSrc = useMemo(
    () => (files[0] ? URL.createObjectURL(files[0]) : undefined),
    [files]
  );
  const previewCoverSrc = newCoverBlobSrc ?? existingCoverSrc;
      ...
      section: (
        <div className="flex w-full flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <UploadImage
              file={files[0]}
              existingImageSrc={existingCoverSrc}
              openRef={openRef}
              onDrop={handleDrop}
              ...
              error={
                warningAlert && files.length === 0 && !cover
      ...
      section: (
        <div className="flex flex-col gap-5">
          <div className="flex items-start gap-2.5 rounded-xl border bg-muted/30 p-3.5 text-sm text-muted-foreground">
            ...
            Actualiza el contenido en español. En el siguiente paso podrás
    ...
    },
    {
      section: (
        <div className="flex flex-col gap-5">
          <div className="flex items-start gap-2.5 rounded-xl border bg-muted/30 p-3.5 text-sm text-muted-foreground">
            <Info className="mt-0.5 size-4 shrink-0" />
            Ahora actualiza el mismo contenido, pero en inglés.
          </div>
      ...
      <DialogContent className="flex h-[95vh] w-[95vw] max-w-none sm:max-w-none flex-col gap-0 overflow-hidden rounded-3xl border-none p-0 shadow-2xl font-montserrat">
        <div className="px-8 pt-8">
          <Header
            ...
            description="Actualiza la información en español e inglés de esta publicación."
          />
        </div>
