...
import { useSubsectionById, useSubsectionTranspFilter } from '@/services/subsection/service';
...
export default function page({ params }: { params: { sectionId: string, subsectionId: string } }) {
  ...
  const { data: filters, isLoading: filtersLoading } = useSubsectionTranspFilter(params.subsectionId)
  ...
  return (
    <div className='w-11/12 bg-white'>
      <div className='w-full flex flex-col gap-3'>
        <h1 className='text-3xl font-bold font-montserrat text-black line-clamp-3'>{subsection?.name}</h1>
        {/* Presentar la description como innerHTML */}
        <div className='text-black' dangerouslySetInnerHTML={{ __html: subsection?.description }}></div>
