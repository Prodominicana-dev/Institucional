...
import { useSectionById, useSectionTranspFilter } from '@/services/section/service';
...
export default function page({ params }: { params: { sectionId: string } }) {
  ...
  const { data: filters, isLoading: filtersLoading } = useSectionTranspFilter(params.sectionId)
  ...
  useEffect(() => {
    if(!filtersLoading) {
      ...
      const month = filters[0].months.map(({month, name} : any) => ({ value: month, label: name }))
      setMonthSelected(month[month.length - 1])
      setMonths(month)
      const years = filters.map(({year} : any) => ({ value: year, label: year }))
      setYears(years)
    }
  ...
  useEffect(() => {
    ...
    if(section && section.documents) {
      ...
      docs = section.documents.filter((doc: any) => {
        ...
        return date.getFullYear() === yearSelected?.value && date.getMonth() === monthSelected?.value
      })
  ...
  useEffect(() => {
    if (!filtersLoading && filters && filters.length > 0) {
      const monthsOfYearSelected = filters.find((year: any) => year.year === yearSelected?.value)?.months;
      if (monthsOfYearSelected && monthsOfYearSelected.length > 0) {
        const months = monthsOfYearSelected.map(({ month, name }: any) => ({ value: month, label: name }));
        setMonths(months);
  ...
  return (
    <div className='w-11/12 bg-white'>
      <div className='w-full flex flex-col gap-3'>
        <h1 className='text-3xl font-bold font-montserrat text-black line-clamp-3'>{section?.name}</h1>
        {/* Presentar la description como innerHTML */}
        <div className='text-black' dangerouslySetInnerHTML={{ __html: section?.description }}></div>
        {section?.documents && (
          <>
          <h1 className='font-montserrat text-black font-bold text-lg'>Filtrar</h1>
        ...
        <h1 className='font-montserrat text-black font-bold text-lg'>Documentos</h1>
        <div className='w-full flex flex-col gap-3'>
