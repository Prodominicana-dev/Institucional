...
export default function Page() {
  ...
  const { data, isLoading, refetch } = useDirections();
  ...
  const currentDirections = directions?.slice(
    indexOfFirstDirection,
    indexOfLastDirection
  );
  const totalPages = Math.ceil(directions?.length / itemsPerPage);
  ...
  const handleFilter = () => {
    if (data) {
      ...
      setDirections(filteredData as any);
              ...
              <div className="w-11/12 flex flex-col space-y-4">
                ...
                <div className="w-full  space-y-5 text-black">
                  <>
                    ...
                    {currentDirections?.map((direction: any, key: number) => {
                      return (
                        <Card
                          key={key}
                          direction={direction}
